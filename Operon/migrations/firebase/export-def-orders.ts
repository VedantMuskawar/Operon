/**
 * DEF_ORDERS Export Script - From Legacy Database
 * * Exports complete DEF_ORDERS collection from Legacy (Pave) Firebase project to Excel.
 */
...
function resolveConfig(): ExportConfig {
  ...
  // Updated filename to def-orders
  const outputPath =
    resolvePath(process.env.OUTPUT_PATH) ??
    path.join(process.cwd(), 'data', `def-orders-export-${timestamp}.xlsx`);

  // Updated default collection name to DEF_ORDERS
  const collectionName = process.env.COLLECTION_NAME || 'DEF_ORDERS';
...
async function exportDefOrders() {
  const config = resolveConfig();
  const legacy = initApp(config);
  const legacyDb = legacy.firestore();

  console.log('=== Exporting DEF_ORDERS from Legacy Database ===');
  ...
  console.log(`\nTotal DEF_ORDERS fetched: ${allOrders.length}`);

  if (allOrders.length === 0) {
    console.log('No DEF_ORDERS found in collection:', config.collectionName);
    return;
  }
  ...
  XLSX.utils.book_append_sheet(workbook, worksheet, 'DEF_ORDERS');
  ...
  console.log(`\n=== Export Complete ===`);
  console.log(`Total DEF_ORDERS exported: ${allOrders.length}`);
  console.log(`Output file: ${config.outputPath}`);
}

exportDefOrders().catch((error) => {
