/**
 * DELIVERY_MEMOS Export Script - From Legacy Database
 * 
 * Exports complete DELIVERY_MEMOS collection from Legacy (Pave) Firebase project to Excel.
 */
...
function resolveConfig(): ExportConfig {
  ...
  const outputPath =
    ...
    path.join(process.cwd(), 'data', `delivery-memos-export-${timestamp}.xlsx`);

  const collectionName = process.env.COLLECTION_NAME || 'DELIVERY_MEMOS';
...
async function exportDeliveryMemos() {
  ...
  console.log('=== Exporting DELIVERY_MEMOS from Legacy Database ===');
  console.log('Collection:', config.collectionName);
  console.log('Output file:', config.outputPath);
  console.log('');

  let allDMs: admin.firestore.QueryDocumentSnapshot[] = [];
  ...
  while (true) {
    ...
    allDMs = allDMs.concat(snapshot.docs);
    batchCount += 1;
    lastDoc = snapshot.docs[snapshot.docs.length - 1];
    
    console.log(`Fetched batch ${batchCount}: ${snapshot.size} delivery memos (total: ${allDMs.length})`);
    
    if (snapshot.size < 1000) break;
  }

  console.log(`\nTotal DELIVERY_MEMOS fetched: ${allDMs.length}`);

  if (allDMs.length === 0) {
    console.log('No DELIVERY_MEMOS found in collection:', config.collectionName);
    return;
  }

  const rows = allDMs.map((doc) => flattenDocument(doc.id, doc.data()));
  ...
  console.log(`\n=== Export Complete ===`);
  console.log(`Total DELIVERY_MEMOS exported: ${allDMs.length}`);
  console.log(`Output file: ${config.outputPath}`);
}
