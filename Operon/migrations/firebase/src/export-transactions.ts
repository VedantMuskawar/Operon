/**
 * TRANSACTIONS Export Script - From Legacy Database
 * 
 * Exports complete TRANSACTIONS collection from Legacy (Pave) Firebase project to Excel.
 */
...
function resolveConfig(): ExportConfig {
  ...
  const outputPath =
    ...
    path.join(process.cwd(), 'data', `transactions-export-${timestamp}.xlsx`);
...
async function exportTransactions() {
  ...
  console.log('=== Exporting TRANSACTIONS from Legacy Database ===');
  console.log('Collection:', config.collectionName);
  console.log('Output file:', config.outputPath);
  console.log('');

  let allTransactions: admin.firestore.QueryDocumentSnapshot[] = [];
  ...
  while (true) {
    ...
    allTransactions = allTransactions.concat(snapshot.docs);
    batchCount += 1;
    lastDoc = snapshot.docs[snapshot.docs.length - 1];
    
    console.log(`Fetched batch ${batchCount}: ${snapshot.size} transactions (total: ${allTransactions.length})`);
  ...
  if (allTransactions.length === 0) {
    console.log('No TRANSACTIONS found in collection:', config.collectionName);
    return;
  }

  const rows = allTransactions.map((doc) => flattenDocument(doc.id, doc.data()));
  ...
  console.log(`\n=== Export Complete ===`);
  console.log(`Total TRANSACTIONS exported: ${allTransactions.length}`);
  console.log(`Output file: ${config.outputPath}`);
}

exportTransactions().catch((error) => {
