...
import 'package:operon_auth_flow/src/repositories/user_organization_repository.dart';
...
class OrgSelectorState extends BaseState {
  ...
  OrgSelectorState copyWith({
    ...
    return OrgSelectorState(
      ...
      isFinancialYearLocked: isFinancialYearLocked ?? this.isFinancialYearLocked,
    );
  }
}
...
class OrgSelectorCubit extends Cubit<OrgSelectorState> {
  ...
  void selectOrganization(OrganizationMembership organization) {
    final isAdmin = organization.role.toUpperCase() == 'ADMIN';
    emit(
      state.copyWith(
