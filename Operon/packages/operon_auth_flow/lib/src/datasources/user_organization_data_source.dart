...
class UserOrganizationDataSource {
  ...
  Future<List<UserOrganizationRecord>> fetchUserOrganizations({
    ...
    return snapshot.docs
        .map(
          (doc) => UserOrganizationRecord(
            id: doc.id,
            name: doc.data()['org_name'] as String? ?? 'Untitled Org',
            role: doc.data()['role_in_org'] as String? ?? 'member',
            appAccessRoleId: doc.data()['app_access_role_id'] as String?,
          ),
        )
        .toList();
  }
