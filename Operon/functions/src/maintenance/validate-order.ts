...
export const validateOrder = onCall(async (request) => {
  ...
  items.forEach((item: any, index: number) => {
    ...
    if (!item.productName) warnings.push(`Item ${index}: missing productName`);
    if (item.estimatedTrips < 0) errors.push(`Item ${index}: estimatedTrips cannot be negative`);
    if (item.fixedQuantityPerTrip <= 0) errors.push(`Item ${index}: fixedQuantityPerTrip must be > 0`);
    if (item.scheduledTrips < 0) errors.push(`Item ${index}: scheduledTrips cannot be negative`);
  ...
  const calculatedTotalGst = items.reduce((sum: number, item: any) => sum + (item.gstAmount || 0), 0);
  ...
  const tripRefs = scheduledTrips.map((trip: any) => trip.tripId);
  ...
  items.forEach((item: any, index: number) => {
    const itemTrips = scheduledTrips.filter((t: any) => t.itemIndex === index);
