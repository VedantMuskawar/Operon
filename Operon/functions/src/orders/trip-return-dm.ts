...
export const onTripReturnedCreateDM = onDocumentUpdated(
  ...
  async (event) => {
    ...
    try {
      ...
      // #region agent log
      console.error('[DEBUG] DM update FAILED', {tripId, dmId: dmDoc.id, error: updateError?.message, errorStack: updateError?.stack, hypothesisId: 'C'});
      fetch('http://127.0.0.1:7243/ingest/0f2c904c-02d4-456a-9593-57a451fc7c6a',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'trip-return-dm.ts:85',message:'DM update FAILED',data:{tripId,dmId:dmDoc.id,error:updateError?.message,errorStack:updateError?.stack},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
      // #endregion
      throw updateError;
    }
