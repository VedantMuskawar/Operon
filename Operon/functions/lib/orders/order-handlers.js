...
exports.onPendingOrderCreated = (0, firestore_1.onDocumentCreated)(Object.assign({ document: `${constants_1.PENDING_ORDERS_COLLECTION}/{orderId}` }, function_config_1.STANDARD_TRIGGER_OPTS), async (event) => {
    ...
    const initUpdates = {};
    const existingStatus = orderData.status || '';
    if (!existingStatus) {
    ...
    if (Object.keys(initUpdates).length > 0) {
        await snapshot.ref.update(Object.assign(Object.assign({}, initUpdates), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
    }
