...
exports.onOrderCreatedAutoSchedule = void 0;
exports.removeEstimatedDeliveryDateReference = removeEstimatedDeliveryDateReference;
 ...
 */
function calculateTotalTrips(order) {
...
function getVehicleCapacityForProduct(vehicle, productId) {
    ...
    if ((_a = vehicle.productCapacities) === null || _a === void 0 ? void 0 : _a[productId]) {
...
function getDailyCapacity(vehicle, date) {
    ...
    if ((_a = vehicle.weeklyCapacity) === null || _a === void 0 ? void 0 : _a[dayOfWeek]) {
 ...
 */
async function findBestFitVehicle(vehicles, order, organizationId) {
    ...
    const vehicleMaxAllocated = {};
    pendingOrdersSnapshot.docs.forEach((doc) => {
        const orderData = doc.data();
        const autoSchedule = orderData.autoSchedule;
        if ((autoSchedule === null || autoSchedule === void 0 ? void 0 : autoSchedule.suggestedVehicleId) && doc.id !== order.orderId) {
            const vehicleId = autoSchedule.suggestedVehicleId;
            ...
            if (orderItems.length > 0) {
                ...
                if (itemProductId === productId) {
                    ...
                    const allocatedQty = item.fixedQuantityPerTrip || item.totalQuantity;
                    vehicleMaxAllocated[vehicleId] = Math.max(vehicleMaxAllocated[vehicleId] || 0, allocatedQty);
                }
            }
        }
    });
 ...
 */
async function calculateEstimatedDeliveryDate(organizationId, newOrder, vehicles) {
    if (vehicles.length === 0) {
    ...
    const pendingOrdersSnapshot = await db
        ...
        .get();
    const pendingOrders = pendingOrdersSnapshot.docs.map((doc) => {
        const data = doc.data();
        return {
            orderId: doc.id,
...
function calculatePriorityScore(order) {
    ...
    const daysSinceCreated = Math.floor((Date.now() - order.createdAt.toMillis()) / (1000 * 60 * 60 * 24));
 ...
 */
async function updateEstimatedDeliveryDateReference(organizationId, productId, fixedQuantityPerTrip, estimatedDate) {
    try {
        ...
        const estimatedDeliveryDates = orgData.estimatedDeliveryDates || {};
        ...
        const recentDates = entry.recentDates || [];
 ...
 */
async function removeEstimatedDeliveryDateReference(organizationId, productId, fixedQuantityPerTrip, estimatedDate) {
    try {
        const orgRef = db.collection(constants_1.ORGANIZATIONS_COLLECTION).doc(organizationId);
        const orgDoc = await orgRef.get();
        if (!orgDoc.exists) {
        ...
        const updatedRecentDates = recentDates.filter((ts) => ts.toMillis() !== estimatedDate.toMillis());
 ...
 */
async function autoScheduleOrder(orderId, orderData) {
    ...
    const vehicles = vehiclesSnapshot.docs.map((doc) => {
        ...
        return {
            ...
            productCapacities: data.productCapacities || undefined,
    ...
    try {
        ...
        const estimatedDeliveryDate = await calculateEstimatedDeliveryDate(organizationId, order, vehicles);
        ...
        if (primaryProduct) {
            ...
            pendingOrdersSnapshot.docs.forEach((doc) => {
                if (doc.id === orderId)
                    return; // Skip current order
                const orderData = doc.data();
                const autoSchedule = orderData.autoSchedule;
                if ((autoSchedule === null || autoSchedule === void 0 ? void 0 : autoSchedule.suggestedVehicleId) === suggestedVehicle.id) {
                    const orderItems = orderData.items || [];
                    if (orderItems.length > 0) {
                        const item = orderItems[0];
        ...
        // Update estimated delivery date reference in organization document
        if (primaryProduct) {
            await updateEstimatedDeliveryDateReference(organizationId, primaryProduct.productId, primaryProduct.fixedQuantityPerTrip || primaryProduct.totalQuantity, estimatedDeliveryDate);
        }
...
//# sourceMappingURL=order-scheduling.js.map