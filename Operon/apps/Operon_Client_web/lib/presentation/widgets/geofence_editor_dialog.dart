...
class _GeofenceEditorDialogState extends State<GeofenceEditorDialog> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      debugPrint('GeofenceEditorDialog _loadData error: $e\n$st');
      if (!mounted) return;
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final dialogWidth = (screenWidth * 0.9).clamp(500.0, 900.0);
    ...
    if (_isLoading) {
      ...
      content = _buildEditorContent();
    }

    // Wrap the dialog with BlocProvider if we have organization and location
    final dialog = AlertDialog(
      ...
      title: Text(
        isCreate ? 'Create Geofence' : 'Edit Geofence',
        style: const TextStyle(color: AuthColors.textMain),
      ),
      content: SizedBox(
        width: dialogWidth,
    ...
    // Provide GeofencesCubit at the dialog level if we have organization and location
    if (!_isLoading && _loadError == null && _location != null && organization != null) {
