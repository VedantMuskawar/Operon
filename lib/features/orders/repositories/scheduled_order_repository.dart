...
class ScheduledOrderRepository {
  ...
  Future<List<Vehicle>> fetchEligibleVehicles({
    ...
    final minCapacity = candidates.fold<int>(
      candidates.first.vehicleQuantity,
      (current, vehicle) => math.min(current, vehicle.vehicleQuantity),
    );

    return candidates
        .where((vehicle) => vehicle.vehicleQuantity == minCapacity)
        .toList()
      ..sort(
  ...
  Future<void> appendTripLocation({
    required String scheduleId,
    required TripLocation location,
  }) async {
    final collection = _tripLocationsCollection(scheduleId);
    await collection.add(location.toFirestore());
  }
  ...
  Future<Vehicle?> fetchVehicleById({
    ...
    return _vehicleRepository.getVehicleById(organizationId, vehicleId);
  }
  ...
  Future<List<int>> fetchAvailableSlots({
    ...
    final weekdayKey =
        _weekdayKey(truncatedDate); // e.g., Mon, Tue etc as stored in map
    final maxSlots = vehicle.weeklyCapacity[weekdayKey] ?? 0;
    if (maxSlots <= 0) {
      return const [];
    }
  ...
  Future<List<ScheduledOrder>> fetchSchedulesForOrder({
    required String organizationId,
  ...
  Future<ScheduledOrder?> fetchScheduledOrderById(String scheduleId) async {
  ...
  Future<ScheduledOrder> generateDmNumber({
    ...
    final updatedSchedule = await fetchScheduledOrderById(scheduleId);
  ...
  Future<ScheduledOrder> markAsDispatched({
    ...
    final updatedSchedule = await fetchScheduledOrderById(schedule.id);
    if (updatedSchedule == null) {
